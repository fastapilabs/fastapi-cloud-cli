...
def test_shows_error_when_app_does_not_exist(
    ...
    respx_mock.get(f"/teams/{configured_app.team_id}").mock(
        return_value=Response(200, json=_get_random_team())
    )
...
def test_returns_error_when_team_not_found(
    ...
    respx_mock.get(f"/apps/{configured_app.app_id}").mock(
        return_value=Response(200, json=_get_random_app())
    )
    respx_mock.get(f"/teams/{configured_app.team_id}").mock(return_value=Response(404))
